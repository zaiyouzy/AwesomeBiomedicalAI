...
- **Discover candidates (v2):** run `discover-v2` — a rolling 21-day window with two complementary PubMed passes plus abstracts, first-online date verification, an optional failure-tolerant OpenAlex sweep, a compact review queue (default ≤10) and a full raw audit. See [references/discovery-v2.md](references/discovery-v2.md). Read candidates from the queue only; raw candidates stay in the audit artifact and are never stored in the reviewed-papers ledger. Queue admission requires **both** an AI term and an imaging-modality term, and never queues Nature news/commentary DOIs.
- **AI first-pass review (optional):** run `ai-review` after `discover-v2` to have DeepSeek screen the compact queue (strict JSON: `include` / `exclude` / `needs_human_review` + reason + evidence). Read-only and hard-capped by the `ai` block in [assets/curation-rules.json](assets/curation-rules.json); it never edits the catalogue or the ledger, and a human still records every final decision.
